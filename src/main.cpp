...
int main(int argc, char const *argv[]){
    ...
    while(cap.read(frame)){
        ...
        if (GLPoints_Bed.size() != 0){
            glPushMatrix();
            glDisable(GL_TEXTURE_2D);
            ObjectRender::drawBed(GLPoints_Bed, legColors, topColors, 1, false);
            glEnable(GL_TEXTURE_2D);
            glPopMatrix();
        }
        ...
        if (GLPoints_TV.size() != 0){
            glPushMatrix();
            glDisable(GL_TEXTURE_2D);
            ObjectRender::drawTV(GLPoints_TV, legColors, topColors, 1, false);
            glEnable(GL_TEXTURE_2D);
            glPopMatrix();
        }
        ...
        if (GLPoints_Carpet.size() != 0){
            glPushMatrix();
            glDisable(GL_TEXTURE_2D);
            ObjectRender::drawCarpet(GLPoints_Carpet, legColors, topColors, 1, false);
            glEnable(GL_TEXTURE_2D);
            glPopMatrix();
        }
        ...
        if (GLPoints_Bookshelf.size() != 0){
            glPushMatrix();
            glDisable(GL_TEXTURE_2D);
            ObjectRender::drawBookshelf(GLPoints_Bookshelf, legColors, topColors, 1, false);
            glEnable(GL_TEXTURE_2D);
            glPopMatrix();
        }
